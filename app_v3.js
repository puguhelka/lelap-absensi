...
function renderRecords() {
  ...
  let filtered = rows;
  if (search) filtered = filtered.filter(r => getEmployee(r.employeeId)?.fullName?.toLowerCase().includes(search));
  ...
  document.getElementById("recordsExport")?.addEventListener("click", () => alert("Export data absensi akan terhubung ke backend."));
}

// ── LEAVE ──
function renderLeave() {
  document.getElementById("leaveRows").innerHTML = `<tr><td colspan="9" style="text-align:center;padding:40px;color:var(--muted)">Data izin/sakit/cuti akan terhubung ke backend.</td></tr>`;
}
...
function renderReports() {
  ["reportDailyExcel","reportDailyPdf","reportMonthlyExcel","reportMonthlyPdf","reportPhotoZip"].forEach(id => {
    document.getElementById(id)?.addEventListener("click", () => {
      const labels = { reportDailyExcel: "Export Excel Harian", reportDailyPdf: "Export PDF Harian", reportMonthlyExcel: "Export Excel Bulanan", reportMonthlyPdf: "Export PDF Bulanan", reportPhotoZip: "Export ZIP Foto" };
...
function renderSettings() {
  const office = { name: "Lelap Mom Baby Care Salatiga", address: "Jl. Taman Pahlawan 81, Salatiga", latitude: "-7.330000", longitude: "110.500000", radiusMeter: 20 };
...
async function confirmBulkDelete() {
  ...
  try {
    ...
    const data = res ? await res.json() : {};
    if (res && res.ok) { alert(`✅ ${data.message}`); hideDeleteModal(); await loadAllData(); renderEmployees(); }
...
function openDetail(key) {
  ...
  document.getElementById("detailDate").textContent = `${dayName(parts.slice(1).join("-"))}, ${formatLongDate(parts.slice(1).join("-"))}`;
  document.getElementById("detailContent").innerHTML = `<div class="detail-list"><div class="detail-row"><span>Karyawan</span><span>${emp.fullName}</span></div><div class="detail-row"><span>Jabatan</span><span>${emp.position || "-"}</span></div><div class="detail-row"><span>Tanggal</span><span>${parts.slice(1).join("-")}</span></div><div class="detail-row"><span>Status</span><span>Belum ada data detail.</span></div></div>`;
}
...
function saveCorrection(e) {
  e.preventDefault();
  alert("Koreksi status akan terhubung ke PATCH /api/admin/attendance/{id}/correction.");
  closeCorrection();
}
...
function exportDaily() { alert("Export harian akan terhubung ke backend."); }
function exportMonthly() { alert("Export bulanan akan terhubung ke backend."); }
function exportDetail() { alert("Export detail akan terhubung ke backend."); }
